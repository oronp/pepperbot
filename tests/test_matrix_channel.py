...
import pepperbot.channels.matrix as matrix_module
from pepperbot.bus.events import OutboundMessage
from pepperbot.bus.queue import MessageBus
from pepperbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from pepperbot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("pepperbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "pepperbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("pepperbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "pepperbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
